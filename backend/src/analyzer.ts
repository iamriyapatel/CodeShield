...
import type { AnalysisResult, Finding, Severity, Confidence } from './types.js';
    ...
    source.split(/\r?\n/).forEach((lineText, index) => {
      for (const rule of rules) {
        if (rule.pattern.test(lineText)) {
          findings.push({ id: crypto.randomUUID(), ruleId: rule.id, title: rule.title, description: rule.description, severity: rule.severity, confidence: rule.confidence, file, line: index + 1, evidence: lineText.trim().slice(0, 240), remediation: rule.remediation });
        }
      }
    });
  }
  const posture = findings.some((f) => f.severity === 'Critical') ? 'Critical' : findings.length ? 'Needs attention' : 'Good';
  return { posture, filesAnalyzed: Object.keys(files).length, findings };
}
