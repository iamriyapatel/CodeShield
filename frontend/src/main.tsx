...
function App() { const [source,setSource]=useState(sample); const [result,setResult]=useState<Result|null>(null); const [ai,setAi]=useState(''); const [busy,setBusy]=useState(false); const scan=async()=>{setBusy(true); setAi(''); const local=analyze({'pasted-code.ts':source}); setResult(local); const response=await fetch('/api/llm',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({findings:local.findings})}); const data=await response.json().catch(()=>({})); if(data.choices?.[0]?.message?.content) setAi(data.choices[0].message.content); setBusy(false);}; return <main><header><div><div className="eyebrow">SECURE CODE ANALYSIS</div><h1>Code<span>Shield</span></h1></div><div className="badge">MVP · JS / TS · LOCAL</div></header><section className="hero"><div><h2>See risk before it ships.</h2><p>Paste source code and CodeShield will identify security indicators with evidence and remediation guidance.</p></div><div className={`posture ${result?.posture?.replace(' ','-').toLowerCase() ?? 'idle'}`}>{result?.posture ?? 'Ready to scan'}</div></section><section className="workspace"><div className="panel editor"><div className="panel-title">SOURCE CODE <button onClick={()=>setSource('')}>Clear</button></div><textarea value={source} onChange={e=>setSource(e.target.value)} spellCheck={false}/><button className="scan" onClick={scan} disabled={busy}>{busy?'Analyzing…':'Analyze source'}</button></div><div className="panel"><div className="panel-title">3D THREAT MAP</div><ThreatMap findings={result?.findings ?? []}/><div className="legend"><i className="critical"/>Critical <i className="high"/>High <i className="medium"/>Medium <i className="low"/>Low</div></div></section>{result && <section className="panel findings"><div className="panel-title">FINDINGS <small>{result.findings.length} detected · {result.filesAnalyzed} file</small></div>{result.findings.length===0?<p className="empty">No indicators detected.</p>:result.findings.map(f=><article key={f.id}><div className={`severity ${f.severity.toLowerCase()}`}>{f.severity}</div><div className="finding-body"><h3>{f.title}</h3><p>{f.description}</p><code>{f.file}:{f.line} · {f.evidence}</code><strong>Remediation:</strong> {f.remediation}</div></article>)}</section>}{ai && <section className="panel findings"><div className="panel-title">AI RCA & FIXES</div><div className="finding-body ai-output">{ai}</div></section>}</main> }
